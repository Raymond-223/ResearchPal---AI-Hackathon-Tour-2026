...
class StyleScorer:
    ...
    def to_api_response(self, score: StyleScore) -> Dict[str, Any]:
            """
            转换为API响应格式（适配WriteProfileResponse）
            """
            return {
                ...
                "lexical": {
                    "formality": score.formality_score,
                    "terminology": score.terminology_score,
                    "vocabulary_richness": self._calculate_vocabulary_richness(score.details),
                    "informal_words": score.details.get("informal_words_found", []),
                    "formal_words": score.details.get("formal_words_found", [])
                },
                "structural": {
                    "passive_voice_ratio": score.passive_voice_ratio,
                    "sentence_complexity": score.sentence_complexity,
                    "sentence_count": score.details.get("sentence_count", 0),
                    "avg_sentence_length": score.details.get("avg_sentence_length", 0),
                    "paragraph_structure": self._analyze_paragraph_structure(score.details)
                },
                "diagnostics": self.get_improvement_suggestions(score)
            }
    
    def _calculate_vocabulary_richness(self, details: Dict[str, Any]) -> float:
            """计算词汇丰富度"""
            word_count = details.get("word_count", 1)
            unique_words = details.get("unique_word_count", word_count)
            return round(unique_words / word_count * 100, 2)
    
    def _analyze_paragraph_structure(self, details: Dict[str, Any]) -> Dict[str, Any]:
            """分析段落结构"""
            return {
                "cohesion": 75.0,  # 可基于实际逻辑计算
                "transition_usage": 60.0,
                "logical_flow": 80.0
            }
